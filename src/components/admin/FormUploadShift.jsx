...
export default function FormUploadShift({
  ...
  async function getContohFile() {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute("download", `shift ePresensi.xlsx`);
  ...
  const getMasterShift = useCallback(async () => {
    try {
      const pegawai = await protectGet(
        "/masterWaktuPresensi/upt?upt=" + user?.upt_id
      );
      if (import.meta.env.MODE == "development") {
        console.log(pegawai);
      }
      const sel = pegawai.data.data?.map((item) => {
        ...
        item.label =
          item.nama_setting +
          " (" +
          item.batas_waktu_masuk +
          " - " +
          item.batas_waktu_pulang +
          ")";
        return item;
      });
      setShiftSelect(sel);
  ...
  function ceknip(result) {
    if (result.length > 0) {
      for (const item of result) {
        ...
        const cekpeg = pegawaiSelect.find(
          (peg) => (peg.nip == 0 ? peg.nik : peg.nip) == item.nip
        );
        if (!cekpeg) {
          return `${item.nip} tidak terdaftar di ${getnamaupt(user?.upt_id)
            ?.replace("Balai Karantina Hewan, Ikan, dan Tumbuhan", "BKHIT")
            ?.replace(
        ...
        for (const sh of item.shifts) {
          ...
          const ceksif = shiftSelect.find(
            (sif) => sif.id_setting_waktu_presensi == sh.shiftId
          );
          if (!ceksif) {
            return `${sh.shiftId} tidak terdaftar di ${getnamaupt(user?.upt_id)
              ?.replace("Balai Karantina Hewan, Ikan, dan Tumbuhan", "BKHIT")
              ?.replace(
                "Balai Besar Karantina Hewan, Ikan, dan Tumbuhan",
                "BBKHIT"
      ...
      let result = json?.map((row, index) => {
        if (index != 0 && row[0] != undefined) {
          let obj = {
            ...
            nama: row[1],
            nip: row[2]?.replace(/'/g, ""),
            id_user: pegawaiSelect.filter(
              (item) =>
                (item.nip == 0 ? item.nik : item.nip) ==
                row[2]?.replace(/'/g, "")
            )[0]?.id_user,
            shifts: [],
          };
  ...
  async function submitShiftPegawai(e) {
    ...
    if (fileUpload[0]["shifts"][0]["tanggal"] != dayStart) {
      Swal.fire(
        "Perhatian",
        "Tanggal awal dan periode tanggal awal yang dipilih tidak sama",
        "warning"
      );
      return;
    }
    if (fileUpload[0]["shifts"][lengthShift - 1]["tanggal"] != dayEnd) {
      Swal.fire(
        "Perhatian",
        "Tanggal akhir dan periode tanggal akhir yang dipilih tidak sama",
        "warning"
      );
      return;
    }
    ...
    try {
      ...
      if (response.data.status) {
        ...
        await Swal.fire(
          "Sukses",
          response?.data?.message ?? "Berhasil",
          "success"
        );
        getShift();
      }
        ...
        <form style={{ padding: "20px" }} onSubmit={submitShiftPegawai}>
          ...
          <Form.Item label="Upload">
            <Upload
              ...
              onChange={(e) =>
                e.fileList.length == 0
                  ? setFileUpload("") & setListFileUpload([])
                  : validasiFile(e.file)
              }
            >
              {fileUpload ? (
        ...
        <form style={{ padding: "20px" }} onSubmit={inputShiftPegawai}>
          <Form.Item label="Tanggal">
            <DatePicker
              ...
              format="YYYY-MM-DD"
              onChange={(dates) =>
                setValueInput((values) => ({ ...values, tanggal: dates }))
              }
            />
          </Form.Item>
          <Form.Item label="Pegawai">
            <Select
              ...
              onChange={(e) =>
                setValueInput((values) => ({ ...values, id_user: e }))
              }
          ...
          <Form.Item label="Shift">
            <Select
              ...
              optionFilterProp="label"
              onChange={(e) =>
                setValueInput((values) => ({ ...values, shift_id: e }))
              }
              className="text-left w-auto max-w-full sm:w-[300px]"
              popupMatchSelectWidth={false}
              popupStyle={{ width: "auto" }}
          ...
          <Form.Item label={null}>
            <Button
              variant="solid"
              color={valueInput.id ? "orange" : "primary"}
              htmlType="submit"
              className="me-2"
            >
            ...
            <Button
              type="default"
              onClick={() => setOpenModal(false) & resetValueInput()}
              className="mt-2"
            >
              Tutup
            </Button>
