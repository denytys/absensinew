...
export default function RekapPage() {
    ...
    const getBagian = useCallback(async () => {
        ...
        try {
            ...
            let namaBagian = sel.find(item => item.value == user?.bagian_id)
            setValueSelect(values => ({
                ...values,
                bagianView: namaBagian?.label
            }))
            setBagianSelect(sel)
        } catch (error) {
            if (import.meta.env.MODE == 'development') {
    ...
    const getRekapLaporan = async (data) => {
        ...
        try {
            const response = await protectPostPut("post", "/laporan/rekap", valueSelect)
    ...
    return (
        <div className='mb-8'>
            ...
            <div className="relative overflow-x-auto shadow-md rounded-xl" style={{ display: datatabel ? "block" : "none"}}>
                ...
                <table className="w-full text-sm text-center text-gray-500 dark:text-gray-400">
                    <thead className="text-xs text-black font-bold uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" colSpan={tanggalan?.length} className="py-2 px-2 border-1">
                        ...
                        {datatabel ?
                            datatabel?.map((tab, index) => (
                                <tr key={index} className={cekGanjil(index) ? "odd:bg-white odd:dark:bg-gray-900 text-black even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200" : "odd:bg-white odd:dark:bg-gray-900 text-black even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200"}>
                                    ...
                                    <td className={'text-left px-1 py-1 font-medium border-1 text-gray-900 whitespace-nowrap' + (cekGanjil(index) ? " bg-gray-100" : "")}>{(pegawaiSelect ? pegawaiSelect?.find(e => e.nip == tab[0])?.label : user?.nama)?.toUpperCase()}</td>
